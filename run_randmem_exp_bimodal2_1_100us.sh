...
#SERVER_LAYOUT_LIST=("cFCFS" "dFCFS" "cFCFS+cFCFS" "dFCFS+dFCFS" "dFCFS+dFCFS_2")
SERVER_LAYOUT_LIST=("dFCFS+dFCFS" "dFCFS+dFCFS_2")
...
SERVER_APPLICATION_ITERATIONS_1=200
SERVER_APPLICATION_ITERATIONS_2=16000
SERVER_APPLICATION_MODE=0.995
SERVER_APPLICATION_DISTRIBUTION="bimodal2"
...
for l in ${SERVER_LAYOUT_LIST[@]}; do
	if [ $l = "cFCFS" ]; then
		CLIENT_FLOWS=4
	elif [ $l = "cFCFS+cFCFS" ]; then
		CLIENT_FLOWS=4
		SERVER_NUMBER_OF_CORES="7"
	elif [ $l = "dFCFS+dFCFS" ]; then
		CLIENT_FLOWS=8
		SERVER_NUMBER_OF_CORES="2 6"
	elif [ $l = "dFCFS+dFCFS_2" ]; then
		CLIENT_FLOWS=8
	...
	while [ ${STOP} -eq 0 ]; do
		DIR="randmem/${CLIENT_INTERARRIVAL}_${SERVER_APPLICATION_DISTRIBUTION}_${SERVER_SERVICE_TIME}/$l/${CLIENT_CURRENT_RATE}"
		rm -rf $DIR 
		mkdir -p $DIR 
		...
		for j in `seq 0 $RUNS`; do
			...
			ssh ${SERVER_IP} "cd FINAL/$l/demikernel; sh ./run_server.sh '${SERVER_CORES}' '${SERVER_SCRIPT_ARGS}' ${SERVER_TIMEOUT} ${PERF}" 1>/dev/null 2>/dev/null &
			#ssh ${SERVER_IP} "cd FINAL/$l/demikernel; sh ./run_server.sh '${SERVER_CORES}' '${SERVER_SCRIPT_ARGS}' ${SERVER_TIMEOUT} ${PERF}" &
...
for l in ${SERVER_LAYOUT_LIST[@]}; do
	...
	OUTPUT_FILE="randmem/${CLIENT_INTERARRIVAL}_${SERVER_APPLICATION_DISTRIBUTION}_${SERVER_SERVICE_TIME}/$l.dat"
