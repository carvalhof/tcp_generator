...
CLIENT_OFFSET_VALUES=(0 750 1285)
CLIENT_OFFSET_NAMES=("low" "medium" "high")
...
for l in ${SERVER_LAYOUT_LIST[@]}; do
	...
	for i in `seq 0 ${CLIENT_OFFSET_RANGE}`; do
		...
		for j in `seq 0 $RUNS`; do
			...
			sudo LD_LIBRARY_PATH=${HOME}/lib/x86_64-linux-gnu timeout ${SERVER_TIMEOUT} ./build/tcp-generator -a ${CLIENT_PCI_NIC} -n 4 -l ${CLIENT_CORES} -- -f ${CLIENT_FLOWS} -s ${CLIENT_SIZE} -t ${CLIENT_DURATION} -e ${SEED} -c ${CLIENT_CONF_FILE} -o ${CLIENT_OUTPUT_FILE} -C ${CLIENT_CSV_FILE} -i ${VALUE} -a ${SERVER_APPLICATION} >/dev/null 2>/dev/null &
